...
export default function Contact() {
    ...
    const getIconColorClass = (color) => {
        const colorMap = {
            ...
            sakura: "bg-sakura-50 dark:bg-sakura-900/30 text-sakura-600 dark:text-sakura-400"
        };
        ...
        <div className="grid sm:grid-cols-2 gap-4">
            {contacts.map((contact, index) => (
                <a
                    ...
                    className="washi-card hover:shadow-shoji flex items-center group"
                >
                    <div className={`flex items-center justify-center h-10 w-10 rounded-full ${getIconColorClass(contact.color)} mr-4 transition-transform group-hover:scale-110 duration-300`}>
                        {contact.icon}
                    </div>
                    <span className="text-gray-800 dark:text-gray-200 font-medium group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                        {contact.name}
                    </span>
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 ml-auto text-gray-400 transition-transform group-hover:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
